...
**敏捷开发为代表的持续开发模式**：Dev + Ops

- 虚拟机环境构建工具：自动化构建运行环境
  - Vagrant：基于Virtualbox的虚拟机，用于管理虚拟机生命周期的命令行实用程序；
  - Docker：轻量级容器技术
- 构建工作通用化（基础设置配置管理工具）：统一格式描述，消除环境构建步对个人的依赖
  - **Ansible**：声明式、抽象化、收敛性、幂等性、可移植性，无需 Agent；
  - **Chef**：需要 Agent
- 基础设施测试：代码化自动化
  - [**inspec**](https://github.com/inspec/inspec)：基于ruby，能够**验证配置管理工具的配置结果**
- 
